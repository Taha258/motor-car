...
function CarCard({ car }) {
  return (
    <Link href={`/car/${car.slug.current}`} className="group block bg-[#0f172a] rounded-xl overflow-hidden">
      ...
      <div className="px-4 pb-4">
        <span className="block w-full bg-[#31b56b] text-white text-sm font-semibold text-center py-3 rounded-lg group-hover:bg-[#28a05e] transition-colors">
          SEE DETAILS
        </span>
      </div>
    </Link>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-[#0f172a] text-2xl font-bold uppercase tracking-wide" style={{ fontFamily: 'Syne, sans-serif' }}>
            {activeFilters.length > 0 ? 'FILTERED CARS' : 'ALL CARS'}
          </h2>
        ...
        {activeFilters.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-8">
            {activeFilters.map((f) => (
              <span key={f} className="bg-[#31b56b]/20 text-[#31b56b] text-xs font-medium px-3 py-1 rounded-full">
                {f}
              </span>
            ))}
        ...
        {filtered.length === 0 && (
          <div className="text-center py-16">
            ...
            <Link href="/showroom" className="bg-[#31b56b] text-white px-6 py-3 rounded-lg text-sm font-semibold hover:bg-[#28a05e] transition-colors">
              Sab Cars Dekhein
            </Link>
          </div>
        )}
